#!/bin/bash
# All the real installation logic goes here.
...
if [ -z "$1" ]; then
    ...
    echo -e "     ${green}.::::. SENSI-TUNNEL .::::.${neutral}"
    echo -e "${orange}─────────────────────────────────────────${neutral}"
...
cf_id="jubairff90@gmail.com"
cf_key="A74wQoZuEay_dRkIOL66mZ46ENcELSH0MqcxKddB"
ip=$(wget -qO- ipinfo.io/ip)
key=$(curl -s "https://raw.githubusercontent.com/hackershimul07/access/refs/heads/main/$ip")
nginx_key_url="https://nginx.org/keys/nginx_signing.key"
dropbear_init_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/dropbear/dropbear"
dropbear_conf_url="https://raw.githubusercontent.com/jubairbro/L2TP/refs/heads/main/xtra/dropbear"
dropbear_dss_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/dropbear/dropbear_dss_host_key"
sshd_conf_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/sshd"
banner_url="https://raw.githubusercontent.com/jubairbro/L2TP/refs/heads/main/banner"
common_password_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Dexter/common-password"
ws_py_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Websocket/ws.py"
client_service_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Slowdns/client"
server_service_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/server"
gotop_url="https://raw.githubusercontent.com/xxxserxxx/gotop/refs/heads/master/scripts/download.sh"
haproxy_cfg_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Haproxy/haproxy.cfg"
xray_conf_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/nginx/xray.conf"
udp_url="https://raw.githubusercontent.com/jubairbro/Paid/main/porx/udp-custom-linux-amd64"
nginx_conf_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/nginx/nginx.conf"
badvpn_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/BadVPN-UDPWG/badvpn"
openvpn_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Dexter/openvpn.zip"
vmess_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Dexter/VMess-VLESS-Trojan+Websocket+gRPC/vmess/config.json"
vless_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Dexter/VMess-VLESS-Trojan+Websocket+gRPC/vless/config.json"
trojan_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Dexter/VMess-VLESS-Trojan+Websocket+gRPC/trojan/config.json"
shadowsocks_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Dexter/VMess-VLESS-Trojan+Websocket+gRPC/shadowsocks/config.json"
bbr_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/bbr.sh"
dnstt_client_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Slowdns/dnstt-client"
dnstt_server_url="https://raw.githubusercontent.com/hackershimul07/Faw/refs/heads/main/folder/SuperX/Slowdns/dnstt-server"
file_dir="https://raw.githubusercontent.com/hackershimul07/Faw/main/folder/SuperX/Fuck-system/src.zip"
...
if ! apt update -y; then
    echo -e "${red}Failed to update package list${neutral}"
fi
...
if ! apt-get upgrade -y; then
    echo -e "${red}Failed to upgrade system${neutral}"
else
    echo -e "${green}System upgraded successfully${neutral}"
fi

if ! apt dist-upgrade -y; then
    echo -e "${red}Failed to perform dist-upgrade${neutral}"
else
...
if ! dpkg -s vnstat >/dev/null 2>&1; then
    ...
    if grep -q 'Interface "eth0"' /etc/vnstat.conf; then
        ...
        echo -e "${yellow}Interface eth0 not found in /etc/vnstat.conf${neutral}"
    fi
...
rm -f /root/vnstat-2.9.tar.gz >/dev/null 2>&1 || echo -e "${red}Failed to delete vnstat-2.9.tar.gz file${neutral}"
...
if [ -n "$file_dir" ]; then
    ...
    chmod +x /tmp/src/* >/dev/null 2>&1 || echo -e "${red}Failed to set execute permissions for files in /tmp/src${neutral}"
...
if [ -n "$banner_url" ]; then
    wget -q -O /etc/Jubair.txt "$banner_url" && chmod +x /etc/Jubair.txt >/dev/null 2>&1 || echo -e "${red}Failed to download Jubair.txt${neutral}"
else
    echo -e "${yellow}banner_url is not set, skipping download of Jubair.txt${neutral}"
fi
...
if [ -n "$dnstt_server_url" ]; then
    wget -O /etc/slowdns/dnstt-server "$dnstt_server_url" >/dev/null 2>&1 && chmod +x /etc/slowdns/dnstt-server || echo -e "${red}Failed to download or set execute permission for dnstt-server${neutral}"
else
    echo -e "${yellow}dnstt_server_url is not set, skipping download of dnstt-server${neutral}"
fi

if [ -n "$dnstt_client_url" ]; then
    wget -O /etc/slowdns/dnstt-client "$dnstt_client_url" >/dev/null 2>&1 && chmod +x /etc/slowdns/dnstt-client || echo -e "${red}Failed to download or set execute permission for dnstt-client${neutral}"
else
    echo -e "${yellow}dnstt_client_url is not set, skipping download of dnstt-client${neutral}"
fi

if [ -f /etc/slowdns/dnstt-server ]; then
    /etc/slowdns/dnstt-server -gen-key -privkey-file /etc/slowdns/server.key -pubkey-file /etc/slowdns/server.pub || echo -e "${red}Failed to generate keys for dnstt-server${neutral}"
else
    echo -e "${red}dnstt-server not found, skipping key generation${neutral}"
fi

if [ -f /etc/pam.d/common-password ]; then
    chmod +x /etc/pam.d/common-password || echo -e "${red}Failed to set execute permission for common-password${neutral}"
else
    echo -e "${yellow}/etc/pam.d/common-password not found, skipping permission change${neutral}"
fi

if curl -s "$gotop_url" | bash; then
    chmod +x gotop && sudo mv gotop /usr/local/bin/ || echo -e "${red}Failed to set execute permission or move gotop${neutral}"
else
...
if wget -O /etc/xray/vmess/config.json "$vmess_url" >/dev/null 2>&1; then
    echo -e "${green}Successfully downloaded vmess config${neutral}"
else
    echo -e "${red}Failed to download vmess config${neutral}"
fi

if wget -O /etc/xray/vless/config.json "$vless_url" >/dev/null 2>&1; then
    echo -e "${green}Successfully downloaded vless config${neutral}"
else
    echo -e "${red}Failed to download vless config${neutral}"
fi

if wget -O /etc/xray/trojan/config.json "$trojan_url" >/dev/null 2>&1; then
    echo -e "${green}Successfully downloaded trojan config${neutral}"
else
    echo -e "${red}Failed to download trojan config${neutral}"
fi

if wget -O /etc/xray/shadowsocks/config.json "$shadowsocks_url" >/dev/null 2>&1; then
    echo -e "${green}Successfully downloaded shadowsocks config${neutral}"
else
    echo -e "${red}Failed to download shadowsocks config${neutral}"
fi
...
if iptables -t nat -A POSTROUTING -s 10.8.0.0/24 -o "$interface" -j MASQUERADE; then
    ...
    echo -e "${red}Failed to add POSTROUTING iptables rule for 10.8.0.0/24${neutral}"
fi

if iptables -t nat -A POSTROUTING -s 20.8.0.0/24 -o "$interface" -j MASQUERADE; then
    iptables-save >/etc/iptables/rules.v4
else
    echo -e "${red}Failed to add POSTROUTING iptables rule for 20.8.0.0/24${neutral}"
fi
...
if ! command -v xray >/dev/null 2>&1; then
    ...
    wget -qO /usr/local/bin/xray "https://github.com/SonzaiEkkusu/tunnel/releases/download/v1/xray.linux.64bit" && \
...
# Function to add certificates to OpenVPN configurations
input_cert_ovpn() {
...
if ! zip -j /var/www/html/allovpn.zip /var/www/html/*.ovpn; then
    echo -e "${red}Failed to create zip file for all client configurations${neutral}"
fi

# Enable autostart for OpenVPN
if ! sed -i 's/#AUTOSTART="all"/AUTOSTART="all"/g' /etc/default/openvpn; then
    echo -e "${red}Failed to enable autostart for OpenVPN${neutral}"
fi
...
# Create cron job to run /usr/bin/exp daily at 02:00
cat >/etc/cron.d/exp_daily <<EOF
SHELL=/bin/sh
...
# Create cron job to reboot the system daily at 05:00
...
# Create cron job to clear logs every 6 hours
...
# Save daily reboot time
cat >/home/daily_reboot <<EOF
5
EOF

# Define service configuration variables
...
documentation="Documentation=https://t.me/SensiTunnell"
...
create_service "vmess" "SensiTunnel Server Xray Instance" "/usr/local/bin/xray run -config /etc/xray/vmess/%i.json"
create_service "vless" "SensiTunnel Server Xray Instance" "/usr/local/bin/xray run -config /etc/xray/vless/%i.json"
create_service "trojan" "SensiTunnel Server Xray Instance" "/usr/local/bin/xray run -config /etc/xray/trojan/%i.json"
create_service "shadowsocks" "SensiTunnel Server Shadowsocks Instance" "/usr/local/bin/xray run -config /etc/xray/shadowsocks/%i.json"
...
# Reload daemon and manage services
...
[Unit]
Description=Python Proxy SensiTunnel
Documentation=https://t.me/Jubairbro_bot
${after}
...
Documentation=https://t.me/JubairFF
After=syslog.target network-online.target
...
setup_bot() {
    ...
    # Check and install npm dependencies if not already installed
    ...
    # Download bot.zip if app.js doesn't exist
    if [ ! -f /root/.bot/app.js ]; then
        wget -q -O /root/.bot/bot.zip https://raw.githubusercontent.com/hackershimul07/Faw/main/folder/SuperX/Fuck-system/bot.zip
        unzip -o /root/.bot/bot.zip -d /root/.bot >/dev/null 2>&1
        rm /root/.bot/bot.zip >/dev/null 2>&1
    fi

    # Set execute permissions for all files in .bot directory
...
print_rainbow "───────────────────────────────────────"
echo -e "${green}Congratulations! Installation completed successfully${neutral}"
echo -e "${green}Please reboot your server by pressing 'Enter'.${neutral}"
print_rainbow "───────────────────────────────────────"
rm -rf "$0"
